...
class Canvas {
 ...
 public:
   ...
   */
  Canvas(size_t w = 0, size_t h = 0, int bs = 4);
  /// Destructor.
  ~Canvas(void);
  /// Copy constructor.
  Canvas(const Canvas&);
  ...
  size_t width(void) const { return m_r_width; }
  /// Get the canvas height.
  size_t height(void) const { return m_r_height; }
 ...
 private:
  ...
  size_t m_r_width;              //!< The real image width in pixel units.
  size_t m_r_height;             //!< The real image height in pixel units.
  int m_block_size;            //!< Cell size in pixels
